...
class Encoder(common.Module):
  ...
  def __init__(
    ...
    if isinstance(cnn_keys, str) and ',' in cnn_keys and not any(c in cnn_keys for c in r'.*+?$^[]{}()|'):
        ...
        self._cnn_keys = [
            k for k, v in shapes.items() if k in allowed_cnn_keys and len(v) >= 3]
    else:
        self._cnn_keys = [
    ...
    if isinstance(mlp_keys, str) and ',' in mlp_keys and not any(c in mlp_keys for c in r'.*+?$^[]{}()|'):
        ...
        self._mlp_keys = [
            k for k, v in shapes.items() if k in allowed_mlp_keys and len(v) < 3]
    else:
        self._mlp_keys = [
            k for k, v in shapes.items() if re.match(mlp_keys, k) and len(v) < 3]

    print(f"[DEBUG Encoder.__init__] Received shapes: {shapes}")
    print(f"[DEBUG Encoder.__init__] cnn_keys pattern/list: '{cnn_keys}', mlp_keys pattern/list: '{mlp_keys}'")
    print(f"[DEBUG Encoder.__init__] Matched CNN keys: {self._cnn_keys}")
    print(f"[DEBUG Encoder.__init__] Matched MLP keys: {self._mlp_keys}")
    if self._cnn_keys:
  ...
  def __call__(self, data):
    ...
    if not outputs:
        tf.print("[ERROR Encoder.__call__] No features generated by CNN or MLP!", summarize=-1)
        tf.print("[ERROR Encoder.__call__] Input data keys:", list(data.keys()), summarize=-1)
        tf.print("[ERROR Encoder.__call__] Expected CNN keys:", self._cnn_keys, summarize=-1)
        tf.print("[ERROR Encoder.__call__] Expected MLP keys:", self._mlp_keys, summarize=-1)
...
class Decoder(common.Module):
  ...
  def __init__(
      self, shapes, cnn_keys='image', mlp_keys='$^', act='elu', norm='none',
    ...
    self._shapes = shapes
    self.cnn_keys = re.compile(cnn_keys)  # Store cnn_keys
    self.mlp_keys = re.compile(mlp_keys)  # Store mlp_keys
    if self.cnn_keys == self.mlp_keys:
      raise ValueError("CNN and MLP keys cannot be the same.")
    self._act = get_act(act)
    self._norm = norm
    ...
    # Debug prints during initialization
    print(f"[DEBUG Decoder.__init__] Received shapes: {shapes}")
    print(f"[DEBUG Decoder.__init__] cnn_keys pattern/list: '{cnn_keys}', mlp_keys pattern/list: '{mlp_keys}'")

    # Determine which keys are handled by CNN and MLP based on regex matching
    cnn_matched_keys = [k for k in shapes if self.cnn_keys.match(k)]
    mlp_matched_keys = [k for k in shapes if self.mlp_keys.match(k)]

    print(f"[DEBUG Decoder.__init__] Matched CNN keys: {cnn_matched_keys}")
    print(f"[DEBUG Decoder.__init__] Matched MLP keys: {mlp_matched_keys}")

    # Store the matched keys (optional but good for clarity)
    self._cnn_outputs = cnn_matched_keys
    self._mlp_outputs = mlp_matched_keys

    print(f"Decoder CNN outputs: {self._cnn_outputs}")

  def __call__(self, features):
    features = tf.cast(features, prec.global_policy().compute_dtype)
    outputs = {}
    if self._cnn_outputs:
      outputs.update(self._cnn_decoder(features))
    if self._mlp_outputs:
    ...
    def __call__(self, features):
        ...
        for i, kernel in enumerate(self._kernels):
            ...
            if i == len(self._kernels) - 1:
                depth = sum(self._shapes[k][-1] for k in self._shapes)
                act = tf.identity
                norm = 'none'
        ...
        means = tf.split(x, [self._shapes[k][-1] for k in self._shapes], axis=-1)
        dists = {
            key: tfd.Independent(tfd.Normal(mean, 1), 3)
            for key, mean in zip(self._shapes.keys(), means)
        }
        return dists
...
class MLP(common.Module):
  ...
  def __call__(self, features):
    ...
    if self._shape is None:
        if needs_flatten:
             x = tf.reshape(x, tf.concat([original_batch_dims, [tf.shape(x)[-1]]], axis=0)) # Use tf.concat for dynamic shape
        return x
    else:
        dists = {}
        ...
        for key, shape in output_shapes.items():
            ...
            if needs_batch_reshape:
                 new_batch_shape = tf.concat([original_batch_dims, base_dist.batch_shape_tensor()[len(original_batch_dims):]], axis=0) # Dynamic concat
